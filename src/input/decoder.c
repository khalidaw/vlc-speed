...
void *ManageClocksThread(void *args) {
	...
	case threadAudio:
		...
		strcpy(filePath ,"/home/vlc/simpleMaskAudio");
		break;
	case threadVideo:
		currentClock = videoClock;
		strcpy(filePath ,"/home/vlc/simpleMaskVideo");
		break;
	default:
		assert(false);
		break;
	...
	while (ret > 0) {
		...
		float input;
		ret = fscanf(fp, "%s %f\n", command, &input);
		printf("%s in %s", command, type == threadAudio ? ":Audio\n" : ":Video\n");

		if (ret <= 0) {
			printf("%s in %s", "Last cmd", type == threadAudio ? ":Audio\n" : ":Video\n");
			break;
		}
...
//				&& p_owner->fmt_description.i_cat == VIDEO_ES) {
//        	if(counter % 50 == 0)
//			input_clock_ChangeRate(p_owner->p_clock, counter);
//        	clock_Set_delay(p_owner->p_clock, 0);
//        	for(int j = 0 ; j< 2;j ++){
...
//        while(size > 0 && (i < 2)){
//        	p_block = block_FifoGet( p_owner->p_fifo );
//        	size = block_FifoSize(p_owner->p_fifo);
//        	i++;
//        }
