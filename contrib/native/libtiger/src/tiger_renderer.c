...
static void tiger_renderer_init_defaults(tiger_defaults *defaults)
{
  ...
  defaults->font_effect=tiger_font_plain;
  defaults->font_effect_strength=0.0;
}
...
/**
  \ingroup renderer
  Sets a cairo surface where to render the Kate stream
  \param tr the Tiger renderer for which to set the buffer
  \param cs the cairo surface to render to
  \returns 0 success
  \returns TIGER_E_* error
  */
int tiger_renderer_set_cairo_surface(tiger_renderer *tr,cairo_surface_t *cs)
{
