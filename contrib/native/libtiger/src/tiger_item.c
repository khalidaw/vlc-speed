...
static void tiger_item_setup_filter_quality(const tiger_item *ti,cairo_pattern_t *cp)
{
...
static void tiger_item_draw_background_bitmap(tiger_item *ti,cairo_t *cr,const kate_bitmap *kb,const kate_palette *kp,int scale_to_region)
{
  cairo_surface_t *cs;
  int ret;

  ret=tiger_bitmap_cache_get(&ti->tbc,kb,kp,&cs);
  if (ret>=0 && cs) {
    ...
    cp=cairo_get_source(cr);
    cairo_pattern_get_matrix(cp,&matrix);

    if (kin->has.bitmap_size) {
      cairo_matrix_scale(&matrix,kb->width/kin->bitmap_size_x,kb->height/kin->bitmap_size_y);
    }
    else if (scale_to_region && ti->explicit_region) {
      /* if we have an explicit region, the bitmap is scaled to it (without margins) */
      cairo_matrix_scale(&matrix,kb->width/(ti->region.x1-ti->region.x0),kb->height/(ti->region.y1-ti->region.y0));
    }
    else if (kin->ki->original_canvas_width>0 && kin->ki->original_canvas_height>0) {
      cairo_matrix_scale(&matrix,ti->frame_width/(double)kin->ki->original_canvas_width,ti->frame_height/(double)kin->ki->original_canvas_height);
    }
    ...
    cairo_pattern_set_matrix(cp,&matrix);
    tiger_item_setup_filter_quality(ti,cp);
...
static void tiger_item_font_mapping_renderer(cairo_t *cr,PangoAttrShape *attr,gboolean do_path,gpointer data)
{
  ...
  if (cs) {
    ...
    cairo_matrix_translate(&matrix,-attr->logical_rect.x/(double)PANGO_SCALE,-attr->logical_rect.y/(double)PANGO_SCALE);
    cairo_pattern_set_matrix(cp,&matrix);
    tiger_item_setup_filter_quality(ti,cp);
  }
  else {
    tiger_log(tiger_log_warning,"Failed to create bitmap\n");
